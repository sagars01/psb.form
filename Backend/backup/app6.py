...
def pre_questions():
	...
	if request.method == 'GET':
		...
		for index in range (0,count):
			count1 = cursor.execute("Update map_key_to_pre set response = %s where question_id = %s and key_id = %s",( details[0]['Questionaires'][index]['Question_response'], details[0]['Questionaires'][index]['Question_id'], key_id[0][0]))
		if count1 == 0:
				cursor.execute("Insert into map_key_to_pre (key_id, question_id, response) values (%s, %s, %s)",( key_id, details[0]['Questionaires'][index]['Question_response'], details[0]['Questionaires'][index]['Question_id']))
				db.commit();

		count = len(details[0]['Milestones'])
		for index in range (0,count):
                       count1 =  cursor.execute("Update map_key_to_milestones set date = %s where mile_id = %s and key_id = %s",( details[0]['Milestones'][index]['Milestone_response'], details[0]['Milestones'][index]['Milestone_id'], key_id[0][0]))
		if count1 == 0:
                                cursor.execute("Insert into map_key_to_milestones (key_id, date, mile_id) values (%s, %s, %s)",( key_id, details[0]['Milestones'][index]['Milestone_response'], details[0]['Milestones'][index]['Milestone_id']))
		db.commit()
		cursor.execute("Update map_user_to_product set minutes_of_meeting = %s, Phase_details = %s where user_id = %s and product_id = %s",( details[0]['Mom'], details[0]['Phase_details'],userid[0][0], productid))
		db.commit()
		array1.append({"Status": "OK"})
	return jsonify(array1)
...
def psb():
	...
	if request.method == 'GET':
		...
		count = len(details)
		for index in range (0,count):
			count =  cursor.execute("Update map_key_to_psb set status = %s, justification = %s where psb_id = %s and key_id = %s",( details[index]['status'], details[index]['justification'], details[index]['psb_id'], key_id[0][0]))
			if count == 0:
