...
@app.route('/modifypsb', methods=['POST'])
def modify_psb():
        db = MySQLdb.connect( host, user, password, database)
        cursor = db.cursor()
        details = request.get_json(force=True)
	if details['action'] == "delete":
		...
		if details['Request'] != "" or details['Reason'] != "":
                	cursor.execute("Insert into psb (psb_id, req, reason) values (%s, %s, %s)", (details['Id'], details['Request'],details['Reason']))
...
def modify_help():
        ...
        details = request.get_json(force=True)
	if details['action'] == "delete":
                count = cursor.execute("Select * from prequestions where question_id = %s", details['Id'])
                if count > 0:
                        cursor.execute("Delete from prequestions where question_id = %s", details['Milestone_id'])
                count = cursor.execute("Select * from milestone where mile_id = %s", details['Milestone_id'])
                if count > 0:
                        cursor.execute("Delete from milestone where mile_id = %s", details['Milestone_id'])

        elif details['action'] == "add":
                cursor.execute("Insert into prepquestions (question) values (%s)", details['Milestone'])
        cursor.close()
	return jsonify({"Status" : "OK"})
...
def poc():
	...
	if details['product_engineer']:
		cursor.execute ( "Insert into Login (user, is_admin, is_manager) values (%s, %s, %s)",(details['product_engineer'], 0 , 0));
	if details['sdl_engineer']:
		cursor.execute ( "Insert into Login (user, is_admin, is_manager) values (%s, %s, %s)",(details['sdl_engineer'], 1 , 0));
	if details['manager']:
		cursor.execute ( "Insert into Login (user, is_admin, is_manager) values (%s, %s, %s)",(details['manager'], 0 , 1));
	db.commit(i)
	cursor.close()
	return jsonify({"Status" : "OK"})
...
def product_select():
	...
	if request.method == 'GET':
		...
		cursor = db.cursor()
		details = request.get_json(force=True)
		cursor.execute("Select version_id from version where version = %s and product_id = %s",(details['version'], details['product_id']))
